...
import {
  createEditorBlockAction,
  deleteEditorBlockAction,
  duplicateEditorBlockAction,
  publishEditorPageAction,
  reorderEditorBlocksAction,
  saveEditorDocumentAction,
  toggleEditorBlockAction,
} from "./actions";
...
export function Editor({
  ...
  const persist = useCallback(async (document: EditorDocument) => {
    ...
    const result = await saveEditorDocumentAction({
      ...document,
  ...
  async function createBlock(type: string, config: unknown) {
    setBusy(true);
    const result = await createEditorBlockAction({
  ...
  async function reorder(from: number, to: number) {
    ...
    const result = await reorderEditorBlocksAction(
  ...
  return (
    <div className="-mx-4 -my-6 flex min-h-[calc(100dvh-3.5rem)] flex-col overflow-hidden bg-background sm:-mx-6 md:-mx-8 md:-my-8 md:h-[calc(100dvh-4rem)] md:min-h-0">
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        <EditorNavigation
          value={state.section}
          onChange={(section) => dispatch({ type: "section.changed", section })}
        />
        <div className="order-1 flex min-h-[52vh] min-w-0 flex-1 flex-col overflow-hidden lg:order-2 lg:min-h-0">
          <div className="flex h-12 shrink-0 items-center justify-between border-b border-border-subtle bg-surface px-3">
            ...
            >
              <SegmentedControlItem value="mobile" aria-label={t("mobile")}>
                <Smartphone className="size-3.5" />
                <span className="hidden sm:inline">{t("mobile")}</span>
              </SegmentedControlItem>
              <SegmentedControlItem value="tablet" aria-label={t("tablet")}>
                <Tablet className="size-3.5" />
                <span className="hidden sm:inline">{t("tablet")}</span>
              </SegmentedControlItem>
              <SegmentedControlItem value="desktop" aria-label={t("desktop")}>
            ...
            <label className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <select
                ...
                className="h-8 rounded-sm border border-border bg-surface-raised px-2 text-xs"
              >
        ...
        <aside className="order-2 min-h-[26rem] border-t border-border bg-surface lg:order-3 lg:w-[340px] lg:shrink-0 lg:border-t-0 lg:border-l">
          {state.section === "content" ? (
            selectedBlock ? (
              <BlockInspector
                ...
                onBack={() =>
                  dispatch({ type: "block.selected", blockId: null })
                }
              />
            ) : (
              <BlockList
                ...
                busy={busy}
                onSelect={(blockId) =>
                  dispatch({ type: "block.selected", blockId })
                }
                onCreate={createBlock}
                onDuplicate={(id) => void duplicateBlock(id)}
                onToggle={(id, enabled) => void toggleBlock(id, enabled)}
          ...
          {state.saveStatus === "error" ? (
            <div className="m-3 flex items-center justify-between gap-3 rounded-md border border-danger/20 bg-danger-soft p-3 text-xs text-danger">
              <span>{state.error}</span>
              <Button
                size="sm"
                variant="outlineDanger"
                onClick={() => void persist(state.document)}
              >
                <RotateCcw />
                {common("retry")}
              </Button>
