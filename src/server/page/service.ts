...
export async function updateBlock(userId:string,input:{id:string;type:string;config:unknown;enabled?:boolean}){const block=await db.block.findFirst({where:{id:input.id,deletedAt:null,page:{profile:{userId}}}});if(!block)throw new AppError("NOT_FOUND","Block not found");await requireBlockRollout(input.type);const parsed=parseBlockConfig(input.type,input.config);await validateAssetOwnership(userId,input.type,parsed);return db.block.update({where:{id:block.id},data:{type:input.type as never,config:parsed as Prisma.InputJsonValue,enabled:input.enabled}});}
