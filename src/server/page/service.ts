...
import {featureFlagKeys,requireFeatureEnabled} from "~/server/features/flags";
...
async function validateAssetOwnership(userId:string,type:string,config:unknown){
  const value=config as {assetId?:unknown};
  const assetId=type==="IMAGE"||type==="FEATURED_LINK"?value.assetId:undefined;
  if(typeof assetId!=="string")return;
  if(!await db.mediaAsset.findFirst({where:{id:assetId,ownerId:userId,status:"READY",kind:"IMAGE"},select:{id:true}}))throw new AppError("VALIDATION_ERROR","Image asset is unavailable");
}
async function requireBlockRollout(type:string){if(type==="ADULT_LINK")await requireFeatureEnabled(featureFlagKeys.adultLinks);}
export async function createBlock(userId:string,input:{pageId:string;type:string;config:unknown}){await ownedPage(userId,input.pageId);await requireBlockRollout(input.type);const parsed=parseBlockConfig(input.type,input.config);await validateAssetOwnership(userId,input.type,parsed);const config=parsed as Prisma.InputJsonValue;const {grants}=await getUserEntitlements(userId);for(let attempt=0;attempt<3;attempt++){try{return await db.$transaction(async tx=>{const active={pageId:input.pageId,deletedAt:null};const [count,last]=await Promise.all([tx.block.count({where:active}),tx.block.aggregate({where:active,_max:{position:true}})]);if(!resolveEntitlement(grants,"BLOCKS",count).allowed)throw new AppError("PLAN_LIMIT_REACHED","Block limit reached");return tx.block.create({data:{pageId:input.pageId,type:input.type as never,config,position:(last._max.position??-1)+1}});},{isolationLevel:Prisma.TransactionIsolationLevel.Serializable});}catch(error){if(error instanceof Prisma.PrismaClientKnownRequestError&&error.code==="P2034"&&attempt<2)continue;throw error;}}throw new AppError("CONFLICT","Please retry");}
export async function updateBlock(userId:string,input:{id:string;type:string;config:unknown;enabled?:boolean}){const block=await db.block.findFirst({where:{id:input.id,deletedAt:null,page:{profile:{userId}}}});if(!block)throw new AppError("NOT_FOUND","Block not found");if(block.type!==input.type)await requireBlockRollout(input.type);const parsed=parseBlockConfig(input.type,input.config);await validateAssetOwnership(userId,input.type,parsed);return db.block.update({where:{id:block.id},data:{type:input.type as never,config:parsed as Prisma.InputJsonValue,enabled:input.enabled}});}
