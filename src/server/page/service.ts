...
export async function createBlock(userId:string,input:{pageId:string;type:string;config:unknown}){await ownedPage(userId,input.pageId);const parsed=parseBlockConfig(input.type,input.config);await validateAssetOwnership(userId,input.type,parsed);const config=parsed as Prisma.InputJsonValue;const {grants}=await getUserEntitlements(userId);for(let attempt=0;attempt<3;attempt++){try{return await db.$transaction(async tx=>{const [count,last]=await Promise.all([tx.block.count({where:{pageId:input.pageId}}),tx.block.aggregate({where:{pageId:input.pageId},_max:{position:true}})]);if(!resolveEntitlement(grants,"BLOCKS",count).allowed)throw new AppError("PLAN_LIMIT_REACHED","Block limit reached");return tx.block.create({data:{pageId:input.pageId,type:input.type as never,config,position:(last._max.position??-1)+1}});},{isolationLevel:Prisma.TransactionIsolationLevel.Serializable});}catch(error){if(error instanceof Prisma.PrismaClientKnownRequestError&&error.code==="P2034"&&attempt<2)continue;throw error;}}throw new AppError("CONFLICT","Please retry");}
export async function updateBlock(userId:string,input:{id:string;type:string;config:unknown;enabled?:boolean}){const block=await db.block.findFirst({where:{id:input.id,page:{profile:{userId}}}});if(!block)throw new AppError("NOT_FOUND","Block not found");const parsed=parseBlockConfig(input.type,input.config);await validateAssetOwnership(userId,input.type,parsed);return db.block.update({where:{id:block.id},data:{config:parsed as Prisma.InputJsonValue,enabled:input.enabled}});}
export async function deleteBlock(userId:string,id:string){const result=await db.block.deleteMany({where:{id,page:{profile:{userId}}}});if(!result.count)throw new AppError("NOT_FOUND","Block not found");}
export async function setBlockEnabled(userId:string,id:string,enabled:boolean){const result=await db.block.updateMany({where:{id,page:{profile:{userId}}},data:{enabled}});if(!result.count)throw new AppError("NOT_FOUND","Block not found");}
export async function duplicateBlock(userId:string,id:string){const block=await db.block.findFirst({where:{id,page:{profile:{userId}}}});if(!block)throw new AppError("NOT_FOUND","Block not found");return createBlock(userId,{pageId:block.pageId,type:block.type,config:block.config});}
export async function reorderBlocks(userId:string,pageId:string,ids:string[]){await ownedPage(userId,pageId);const existing=await db.block.findMany({where:{pageId},select:{id:true}});if(existing.length!==ids.length||new Set(ids).size!==ids.length||existing.some((b)=>!ids.includes(b.id)))throw new AppError("VALIDATION_ERROR","Invalid block order");await db.$transaction(ids.map((id,position)=>db.block.update({where:{id},data:{position}})));}
