...
export async function setProfileAvatar(userId:string,profileId:string,assetId:string|null){return db.$transaction(async tx=>{const profile=await tx.profile.findFirst({where:{id:profileId,userId},select:{id:true}});if(!profile)throw new AppError("NOT_FOUND","Profile not found");if(assetId){const asset=await tx.mediaAsset.findFirst({where:{id:assetId,ownerId:userId,kind:"IMAGE",variant:"ORIGINAL",status:"READY"},select:{id:true}});if(!asset)throw new AppError("VALIDATION_ERROR","Avatar image is unavailable");}const updated=await tx.profile.update({where:{id:profileId},data:{avatarAssetId:assetId}});await tx.auditLog.create({data:{actorId:userId,actorType:"USER",action:"PROFILE_AVATAR_CHANGED",targetType:"Profile",targetId:profileId,metadata:{assetId}}});return updated;});}
