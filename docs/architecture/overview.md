...
Major model groups are identity (`User`, `Account`, `Session`, `VerificationToken`), public identity (`Profile`, `UsernameHistory`), content (`Page`, `PageDraft`, `Block`, `Theme`), publishing (`PageVersion`, `PagePublication`), SaaS (`Plan`, `Feature`, `PlanEntitlement`, `Subscription`, `UsageCounter`), platform services (`MediaAsset`, analytics, mail delivery), and trust (`AuditLog`, `SecurityEvent`, reports/moderation, deletion).
